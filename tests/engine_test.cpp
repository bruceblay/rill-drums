...
static void checkTrimSettles() {
  for (uint32_t seed : {1u, 5u, 11u}) {
    ...
    for (int64_t i = 1; i <= int64_t(kit::rate) * 40; ++i) {
      ...
      if (i > int64_t(kit::rate) * 15) worst = std::max<int64_t>(worst, std::llabs(error));
    }
...
static void checkTempoChange() {
  for (uint32_t seed : {2u, 3u}) {
    ...
    for (int64_t i = 1; i <= int64_t(kit::rate) * 50; ++i) {
      ...
      engine.trimGrid(int32_t(error / 4));
      if (switchAt >= 0) {
        if (i < switchAt + int64_t(kit::rate) * 8) during = std::max<int64_t>(during, std::llabs(error));
